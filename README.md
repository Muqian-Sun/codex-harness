...
涉及可执行代码的功能分支还必须对相对目标分支新增或修改的可执行行验证聚合覆盖率。覆盖命令要求唯一且可解析为 Git commit 的显式基准，以其与 `HEAD` 的 merge-base 计算已提交、已暂存、未暂存和未跟踪源码的变更行，并运行完整单元测试；未被测试导入的变更源码仍按零覆盖统计，变更可执行行的聚合覆盖率低于 90% 时失败。纯文档或配置 PR 不适用该指标。
